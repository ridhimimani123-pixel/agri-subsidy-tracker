...
async function loadSummaryStats() {
  try {
    ...
    document.getElementById("total-scans").textContent = data.total_scans ?? data.total ?? "0";
...
function setupRadiusForm() {
  ...
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    const dealerCode = document.getElementById("radius-dealer-code").value.trim();
...
async function loadMLAnomalies(dealerCode) {
  ...
  try {
    const res = await fetch(
      `${BACKEND_URL}/api/ml/dealers/${encodeURIComponent(dealerCode)}/daily-anomalies`
    );
    if (!res.ok) {
      const errData = await res.json();
...
function setupGraphControls() {
  ...
  const img = document.getElementById("daily-graph");
  const placeholder = document.getElementById("graph-placeholder");

  btn.addEventListener("click", () => {
    const dealerCode = document
      .getElementById("graph-dealer-code")
      .value.trim();
    if (!dealerCode) return;

    // Append cache-buster so browser reloads image
    const ts = Date.now();
    img.src = `${BACKEND_URL}/api/plots/dealers/${encodeURIComponent(
      dealerCode
    )}/daily.png?t=${ts}`;
...
function init() {
  ...
  document.getElementById("graph-dealer-code").value = "D123";
}
